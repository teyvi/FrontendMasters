...
function getListOfNames(list) {
  ...
  const workersNames = list.map(worker => worker.name)
...
function getAllEngineers(list) {
 const engineersHere = list.filter(worker => worker.jobTitle.includes("Engineer"));


  // Map over the filtered list to return only the names
  const engineerNames = engineersHere.map(worker => worker.name);

  // Return the list of engineer names
